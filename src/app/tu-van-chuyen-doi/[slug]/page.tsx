...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!s) return {};
  return createMetadata({ title: s.title, description: s.summary, path: paths.consultingService(s.slug) });
}
  ...
  return (
    <>
      <PageHero
        ...
        crumbs={[
          { name: "Trang chủ", href: "/" },
          { name: "Tư vấn chuyển đổi", href: paths.consulting },
          { name: s.title },
        ]}
      />
      <Container className="grid gap-12 py-16 lg:grid-cols-3">
        <div className="space-y-10 lg:col-span-2">
          ...
          <Link href={paths.consulting} className="inline-block text-sm font-semibold text-vabix-deep-teal">
            ← Tất cả dịch vụ tư vấn
          </Link>
        </div>
        <aside>
          <div className="border border-vabix-deep-teal/10 bg-white p-6 lg:sticky lg:top-28">
            <LeadForm type="transformation" title="Trao đổi về dịch vụ này" program={s.slug} />
          </div>
        </aside>
      </Container>
