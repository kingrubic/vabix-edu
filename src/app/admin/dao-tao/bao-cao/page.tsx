...
export default async function ReportsPage() {
  ...
  const enrollmentByClass = new Map(
    await Promise.all(classes.map(async (item) => [item.id, await listEnrollments(item.id)] as const)),
  );
  return (
    <div className="space-y-6">
      ...
      {classes.map((item) => {
        const enrollments = (enrollmentByClass.get(item.id) ?? []) as { id: string; full_name: string }[];
        return (
